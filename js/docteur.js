...
/**
 * Affiche la boîte des docteurs - VPages - Section Employer
 * 
 * @return none
 */
function boxDocteur()
{
	var boxDocteur = document.getElementById('boxdocteur'); 
	boxDocteur.style.display = (boxDocteur.style.display == 'block') ? 'none' : 'block';
	return;
}
...
function submitDocteur(event)
{
  ...
  var rep = actionForm('../Php/index.php?EX=insert_docteur', this);

  // Remplace le contenu de <div id="content" par l'affichage des employés
  changeContent('id_sticky_content', '../Php/index.php?EX=docteur', '', 'initContactsDocteur()');
 ...
 * @return none
 */ 
function initContactsDocteur()
{
  // Récupère l'élément <button id="admin">
  var click_button_admin = document.querySelector('#admin_doc');  
  // Pour l'élément <button id="admin">
  // on associe un événement click
  // avec comme fonction associée adminContactsDocteur
  Listener(click_button_admin, 'click', adminContactsDocteur);
  
  // Récupère l'élément <button id="return">
  var click_button_return = document.querySelector('#return_doc');
 ...
 * @return none
 */ 
function adminContactsDocteur()
{
  changeContent('id_sticky_content', '../Php/index.php?EX=adminDocteur', '', 'initAdminDocteur()');
...
function initAdminDocteur()
{
  ...
  var click_button_update = document.querySelector('#update_doc');
  // Pour l'élément <button id="update">
  // on associe un événement click
  // avec comme fonction associée updateContactsDocteur
  Listener(click_button_update, 'click', updateContactsDocteur);
  
			// Récupère l'élément <button id="update">
			var click_button_delete = document.querySelector('#delete_doc');
			// Pour l'élément <button id="update">
			// on associe un événement click
			// avec comme fonction associée updateContactsDocteur
			Listener(click_button_delete, 'click', deleteContactsDocteur);
  
  // Récupère l'élément <button id="normal">
  var click_button_normal = document.querySelector('#normal_doc');
  // Pour l'élément <button id="normal">
  // on associe un événement click
  // avec comme fonction associée une fonction anonyme
  // qui affiche les contacts
  Listener(click_button_normal, 'click', function(){changeContent('id_sticky_content', '../Php/index.php?EX=docteur', '', 'initContactsDocteur()')});
 ...
 */ 
 ...
 function updateContactsDocteur(event)
{
  // Arrête la propagation de l'événement
  stopEvent(event);
  ...
  var name = new Array();
  var value = new Array();
  name['ID_DOCTEUR'] = new Array();
  value['ID_DOCTEUR'] = new Array();
  name['NOM'] = new Array();
  value['NOM'] = new Array();
  ...
  for (var i = 0, k = 0; i < nb_p_all; ++i)
  {
	  ...
	  if (input_all.length)
	  {
	    ...
	    name['ID_DOCTEUR'][k] = 'ID_DOCTEUR[]';
	    value['ID_DOCTEUR'][k] = id_doc;
  ...
  var rep = actionFormDocteur('../Php/index.php?EX=updateDocteur', frm, name, value);
  
  changeContent('id_sticky_content', '../Php/index.php', 'EX=adminDocteur');
...
function deleteContactsDocteur(event)
{
  // Arrête la propagation de l'événement
  stopEvent(event);
  ...
  // Récupération de l'objet généré par JSON.parse()
  // On appelle la fonction actionForm2
  var rep = actionFormDocteur('../Php/index.php?EX=delete_Docteur', frm, name, value);
